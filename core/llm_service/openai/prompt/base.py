...
def chat_prompt_to_text_prompt(
    prompt: OpenAICreateChatPrompt, for_completion: bool = True
) -> str:
    """
...
class Prompt(ABC):
    ...
    def to_formatted_prompt(self):
        """
        Return the actual sample to be passed as the `prompt` field to your model.
...
class CompletionPrompt(Prompt):
    ...
    @staticmethod
    def _render_chat_prompt_as_text(prompt: OpenAICreateChatPrompt) -> str:
...
class ChatCompletionPrompt(Prompt):
    ...
    The format expected by chat models is a list of messages, where each message is a dict with "role" and "content"
    keys.
    """
